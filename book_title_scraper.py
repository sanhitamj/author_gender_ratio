...
class BookScraper:
    ...
    def extract_info_from_soup(self, soup: BeautifulSoup):
        ...
        if len(authors) < len(urls):
            authors += [''] * [len(urls) - len(authors)]

        mapped_books = []
        for title, author, url in zip(titles, authors, urls):
            # make sure the title or author match with the url.
            if title.split()[0].lower() in url.lower() or author.split()[0].lower() in url.lower():
    ...
    def scrape_for_books(self):
        ...
        url = "/Walk-Blue-Fields-Claire-Keegan/dp/0802170498/"
        link = f"{amazon_url}"
        ...
        while counter < self.limit:
            ...
            if counter % 5 == 0:
                ...
                self.logger.info(f"URL settled on = {url}")
